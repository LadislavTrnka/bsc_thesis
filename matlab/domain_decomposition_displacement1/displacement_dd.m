% Displacement_DomainDecomposition (1) - resampling
...
scale=1e-9;
M1 = DomainDecompositionMatrices(NX1,NY,DOMX1,DOMY,'R');
M2 = DomainDecompositionMatrices(NX2,NY,DOMX2,DOMY,'L');
B1 = DomainBoundary(NX1,NY,DOMX1,DOMY,'R');
B2 = DomainBoundary(NX2,NY,DOMX2,DOMY,'L');
SB = SharedBoundary(NX2,NX1,NY);
...
rhs1=DomainRHS(NX1,NY,DOMX1,DOMY,'R');
rhs2=DomainRHS(NX2,NY,DOMX2,DOMY,'L');
rhs = [rhs1
    rhs2
    zeros(size(SB,1),1)
    zeros(size(M1,1),1)
